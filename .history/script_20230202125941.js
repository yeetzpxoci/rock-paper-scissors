...
function playClick(self) {
    const result = playRound(self.id, getComputerChoice());
...
function game() {
    ...
    rock.addEventListener("click", playClick(this.id));

    paper.addEventListener("click", playClick(this.id));

    scissors.addEventListener("click", playClick(this.id));
}
